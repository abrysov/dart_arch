...
import './my_app_routes.dart' show RoutesGroup, aboutRoute, allRoutes;
import './aroute.dart';
...
final mySettingsProvider =
    ChangeNotifierProvider<AppSettings>((ref) => throw UnimplementedError());

class AppSettings extends ChangeNotifier {
  static final _routeNamerRouteMap = {
    for (ARoute route in allRoutes) route.routeName: route
  };

  static Future<AppSettings> create() async {
    debugPrint('app dir=${await getApplicationDocumentsDirectory()}');
    final sharedPref = await SharedPreferences.getInstance();
    final s = AppSettings._(sharedPref);
    await s._init();
    return s;
  }
  ...
  Future<void> _init() async {
    ...
    if (knownRoutes == null || knownRoutes.length < 50) {
      ...
      final allRouteNames = _routeNamerRouteMap.keys.toList()
        ..add(aboutRoute.routeName);
  ...
  List<ARoute> get starredRoutes => [
        for (final String routename in this.starredRoutenames)
          if (_routeNamerRouteMap[routename] != null)
            _routeNamerRouteMap[routename]!
      ];
  ...
  bool isNewRoute(ARoute route) =>
      !(_pref.getStringList(kKnownRoutesKey)?.contains(route.routeName) ??
          false);

  void markRouteKnown(ARoute route) {
    if (isNewRoute(route)) {
  ...
  int numNewRoutes(RoutesGroup group) {
